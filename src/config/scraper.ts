...
export function createScrapingConfig(): ScrapingConfig {
  const config: ScrapingConfigType = ScrapingConfigSchema.parse({
    targetUrl: process.env['TARGET_URL'] || 'https://www.vivantes.de/klinikum-im-friedrichshain/rettungsstelle',
    browser: {
...
export function validateScrapingConfig(config: ScrapingConfig): void {
  try {
    ScrapingConfigSchema.parse(config);
  } catch (error) {
    if (error instanceof z.ZodError) {
      const issues = error.issues.map(issue => `${issue.path.join('.')}: ${issue.message}`);
      throw new Error(`Scraping configuration validation failed: ${issues.join(', ')}`);
    }
    throw error;
  }
}

export function getDefaultScrapingConfig(): ScrapingConfig {
  return {
    targetUrl: 'https://www.vivantes.de/klinikum-im-friedrichshain/rettungsstelle',
    browser: {
      type: 'chromium',
      headless: true,
      timeout: 30000,
    ...
    maxRetries: 3,
    pageTimeout: 30000,
    scraperId: `hospital-scraper-${Date.now()}`,
  };
}